...
func RSAPublicSign(source, key string) ([]byte, error) {
	key = "-----BEGIN PUBLIC KEY-----\n" + key + "\n-----END PUBLIC KEY-----"
	rsas := NewRSASecurity()
	err := rsas.SetPublicKey(key)
	if err != nil {
		return nil, err
	}
	encrypted, err := rsas.PubKeyENCTYPT([]byte(source))
	if err != nil {
		return nil, err
	}
...
func RSAPrivateSign(source, key string) ([]byte, error) {
	key = "-----BEGIN RSA PRIVATE KEY-----\n" + key + "\n-----END RSA PRIVATE KEY-----"
	rsas := NewRSASecurity()
	err := rsas.SetPrivateKey(key)
	if err != nil {
		return nil, err
	}
	encrypted, err := rsas.PubKeyENCTYPT([]byte(source))
...
func RSAPublicDecode(source []byte, key string) ([]byte, error) {
	key = "-----BEGIN PUBLIC KEY-----\n" + key + "\n-----END PUBLIC KEY-----"
	rsas := NewRSASecurity()
	err := rsas.SetPublicKey(key)
	if err != nil {
	...
	decrypted, err := rsas.PubKeyDECRYPT(source)
	if err != nil {
		return nil, err
	}
...
func RSAPrivateDecode(source []byte, key string) ([]byte, error) {
	key = "-----BEGIN RSA PRIVATE KEY-----\n" + key + "\n-----END RSA PRIVATE KEY-----"
	rsas := NewRSASecurity()
	err := rsas.SetPrivateKey(key)
	if err != nil {
	...
	decrypted, err := rsas.PriKeyDECRYPT(source)
	if err != nil {
		return nil, err
	}
	return decrypted, nil
